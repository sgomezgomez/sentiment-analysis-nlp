## Dependencies
from basic_lr_torch import SA_TorchedLogisticRegression, build_freqs
import torch
import nltk
import numpy as np
...
model = SA_TorchedLogisticRegression(freqs)
print(str(datetime.now()) + ': ' + 'Model instantiated')
# Encode
train_encX = model.encode(train_x)
print(str(datetime.now()) + ': ' + 'Training data encoded')
# Train
model.train(train_encX, torch.tensor(train_y).float(), alpha, num_iters, verbose, verb_interval)
